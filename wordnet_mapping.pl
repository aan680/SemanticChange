...
%Run "run" (main_consec and main_vsnow and infer) to get all the triples.
...
test_json:-
    open_read_close_json("scripts/consec-words", Keys),
    open_read_close_json("scripts/consec-years", Years),
    open_read_close_json("scripts/consec-values", Values).
...
main_vsnow:-  %BEFORE RUNNING, CHANGE THE DATE FUNCTION! SET OFFSET TO 1990
 	open_read_close_json("histwords-vsnow-keys", Keys),
    open_read_close_json("histwords-vsnow-values-keys", Years),
    open_read_close_json("histwords-vsnow-values-values-noNaN-null", Values),
    b_setval(offset_formula, '1990'),
    maplist(iterate_over_keys, Keys, Years, Values).

subprop(S):-
    rdf_assert(S, rdfs:subPropertyOf, cwi:'semantic_change').
...
define_semanticchange_decade_property(LexicalEntry, Value, Onsetstart, Offsetstart):-
    ...
    %rdf_assert(LexicalEntry, Predicate, literal(type(xsd:float, Value))).
